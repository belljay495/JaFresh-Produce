...
const App = {
  ...
  init() {
    ...
    // Auto-open checkout if user just logged in with a pending checkout
    if (sessionStorage.getItem('jf_checkout_intent') && DB.getCurrentUser()) {
      sessionStorage.removeItem('jf_checkout_intent');
      setTimeout(() => this.openCheckout(), 600);
    }
  },
    ...
    grid.innerHTML = products.map(p => `
      <div class="product-card" data-id="${p.id}" onclick="App.openProductModal(${p.id})" style="cursor:pointer">
        ...
        <div class="product-body">
          ...
          <div class="product-desc">${p.description.substring(0, 90)}... <span style="color:var(--green-mid);font-weight:600;font-size:0.78rem">Read more</span></div>
  ...
  openCheckout() {
    ...
    if (!user) {
      ...
      sessionStorage.setItem('jf_checkout_intent', '1');
      this.closeCart();
      setTimeout(() => { window.location.href = 'login-customer.html'; }, 800);
      return;
    }
    ...
    const parish = document.getElementById('checkout-parish');
    if (!parish.value && user.parish) parish.value = user.parish;
  ...
  filterProducts(category, btn) {
    ...
    // Clear the search bar when jumping to a category
  ...
  handleSearch() {
    ...
    if (q.length > 1) {
      // Turn off active visual states on filter tabs during a text search
      document.querySelectorAll('.filter-tab').forEach(t => t.classList.remove('active'));
      this.renderProducts('search');
    } else {
      // Re-activate the 'All' tab if they backspace their search term away
  ...
  openProductModal(id) {
    ...
    if (!p) return;
    const farmer = DB.getFarmer(p.farmer_id);
